...
public:
    ...
    bool isSameBinaryTree(Tree& t) {
        return isSameBinaryTreeAux(mRoot, t.mRoot);
    }
    // You can also put your code here
    bool isSameBinaryTreeAux(Node* a, Node* b) {
        if (a == NULL && b == NULL) return true;
        if (a == NULL || b == NULL) return false;
        if (a->data != b->data) return false;
        return isSameBinaryTreeAux(a->left, b->left) && isSameBinaryTreeAux(a->right, b->right);
    }
protected:
